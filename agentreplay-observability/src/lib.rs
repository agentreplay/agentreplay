...
//! Observability for Agentreplay and LLM applications
//!
//! Agentreplay observes itself using its own database (dogfooding).
...
pub struct ObservabilityConfig {
    ...
    /// API key for Agentreplay authentication
...
impl ObservabilityConfig {
    ...
    pub fn from_env() -> Self {
        ...
        if let Ok(enabled) = std::env::var("AGENTREPLAY_ENABLED") {
            config.enabled = enabled.to_lowercase() == "true" || enabled == "1";
        }

        // API key for Agentreplay authentication
        if let Ok(api_key) = std::env::var("AGENTREPLAY_API_KEY") {
            config.api_key = Some(api_key);
        }

        // Agentreplay endpoint
        if let Ok(endpoint) = std::env::var("AGENTREPLAY_ENDPOINT") {
            config.agentreplay_endpoint = endpoint;
        }

        // Project identifier
        if let Ok(project) = std::env::var("AGENTREPLAY_PROJECT") {
            config.project = Some(project);
        }

        // Service name
        if let Ok(service_name) = std::env::var("AGENTREPLAY_SERVICE_NAME") {
            config.service_name = service_name;
        }

        // Environment
        if let Ok(environment) = std::env::var("AGENTREPLAY_ENVIRONMENT") {
...
/// Initialize observability with Agentreplay self-hosting
///
/// Instead of exporting to external OTLP collectors or Langfuse,
/// Agentreplay observes itself by writing traces to its own database.
...
pub fn init_observability(config: ObservabilityConfig) -> Result<()> {
    if !config.enabled {
        tracing::info!("Agentreplay observability disabled");
        return Ok(());
    }

    // Set up basic tracing subscriber without external exporters
    // Traces will be batched and sent to Agentreplay's own REST API
    ...
    tracing::info!(
        ...
        agentreplay_endpoint = %config.agentreplay_endpoint,
        "Agentreplay self-hosting observability initialized"
    );

    // TODO: Set up batching layer that sends traces to Agentreplay's REST API
...
mod tests {
    ...
    async fn test_tracing_setup() {
        ...
        assert_eq!(config.service_name, "agentreplay-app");
    }
}
