...
//! - Settings are persisted to `~/.agentreplay/retention-config.json`

use crate::Agentreplay;
use agentreplay_core::Result;
...
impl RetentionConfig {
    ...
    pub fn save(&self, path: &PathBuf) -> Result<()> {
        ...
        let contents = serde_json::to_string_pretty(self).map_err(|e| {
            AgentreplayError::Internal(format!("Failed to serialize retention config: {}", e))
        })?;
...
impl RetentionManager {
    ...
    pub async fn apply_retention(&mut self, db: &Agentreplay) -> Result<RetentionStats> {
