...
use flowtrace_core::{
    ...
    Experiment, ExperimentResult, FlowtraceError, PromptTemplate, Result, SpanType,
};
use flowtrace_index::{CausalIndex, DistanceMetric, Embedding, VectorIndex};
use flowtrace_storage::UnifiedStorage;
use moka::sync::Cache;
...
/// Main Flowtrace database interface
///
/// **Thread Safety:**
...
///
/// **Storage Backends:**
/// - LSM (default): FlowTrace's native LSM-tree implementation
...
impl Flowtrace {
    /// Open or create a Flowtrace database
    ///
    ...
    /// Internal helper to initialize Flowtrace with provided storage
        ...
        let causal_index = match CausalIndex::with_persistence(&causal_index_path) {
            Ok(index) if index.is_empty() => {
                ...
                index.save_to_disk().map_err(|e| {
                    FlowtraceError::Index(format!("Failed to save causal index: {}", e))
                })?;
    ...
    pub async fn insert_with_vector(&self, edge: AgentFlowEdge, vector: Embedding) -> Result<()> {
        ...
        if !edge.should_not_embed() {
            self.vector_index
                .add(edge_id, vector)
                .map_err(FlowtraceError::InvalidArgument)?;
        }
    ...
    pub fn query_metrics(
        ...
        start_ts: u64,
        end_ts: u64,
    ) -> flowtrace_storage::MetricsBucket {
    ...
    pub fn query_metrics_timeseries(
        ...
        start_ts: u64,
        end_ts: u64,
    ) -> Vec<(u64, flowtrace_storage::MetricsBucket)> {
    ...
    ) -> Vec<flowtrace_storage::MetricsBucketSnapshot> {
    ...
    pub fn query_sharded_timeseries(
        ...
        start_us: u64,
        end_us: u64,
    ) -> Vec<((u64, u16), flowtrace_storage::MetricsBucketSnapshot)> {
    ...
    pub fn get_sharded_summary(
        ...
        end_us: u64,
        _project_id: Option<u16>,
    ) -> flowtrace_storage::ShardedMetricsSummary {
        let bucket = self.storage.query_metrics(0, 0, start_us, end_us);
        flowtrace_storage::ShardedMetricsSummary {
    ...
    pub fn get_path(&self, from: u128, to: u128) -> Result<Vec<AgentFlowEdge>> {
        let path_ids = self
            .causal_index
            .get_path(from, to)
            .ok_or_else(|| FlowtraceError::NotFound(format!("No path from {} to {}", from, to)))?;
    ...
    pub fn mvcc_stats(&self) -> flowtrace_storage::VersionSetStatsSnapshot {
        self.storage.mvcc_stats()
    }

    /// Get LSM storage statistics for health monitoring
    pub fn storage_stats(&self) -> flowtrace_storage::LSMStats {
        self.storage.stats()
    }
        ...
        F: FnOnce(&mut EvalRun),
    {
        ...
        let run = runs
            .get_mut(&run_id)
            .ok_or_else(|| FlowtraceError::NotFound(format!("Run {} not found", run_id)))?;
    ...
    pub fn sync_vector_index(&self) -> Result<()> {
        ...
        self.vector_index.save_to_disk(&vector_path).map_err(|e| {
            FlowtraceError::Index(format!("Failed to save vector index: {}", e))
        })
    }
    ...
    pub fn close(&self) -> Result<()> {
        ...
        if errors.is_empty() {
            ...
            Err(FlowtraceError::Internal(format!(
    ...
    fn persist_eval_datasets(&self, datasets: &HashMap<u128, EvalDataset>) -> Result<()> {
        ...
        let json = serde_json::to_string_pretty(&datasets_vec).map_err(|e| {
            FlowtraceError::Internal(format!("Failed to serialize datasets: {}", e))
        })?;
        std::fs::write(&path, json).map_err(|e| FlowtraceError::Io(e))?;
        Ok(())
    }
    ...
    fn persist_eval_runs(&self, runs: &HashMap<u128, EvalRun>) -> Result<()> {
        ...
        let json = serde_json::to_string_pretty(&runs_vec)
            .map_err(|e| FlowtraceError::Internal(format!("Failed to serialize runs: {}", e)))?;
        std::fs::write(&path, json).map_err(|e| FlowtraceError::Io(e))?;
        Ok(())
    }
    ...
    pub fn persist_prompt_templates(&self, templates: &HashMap<u128, PromptTemplate>) -> Result<()> {
        ...
        let json = serde_json::to_string_pretty(&templates_vec).map_err(|e| {
            FlowtraceError::Internal(format!("Failed to serialize prompt templates: {}", e))
        })?;
        std::fs::write(&path, json).map_err(|e| FlowtraceError::Io(e))?;
...
pub struct DatabaseStats {
    pub storage: flowtrace_storage::LSMStats,
...
impl QueryBuilder {
    pub fn new(db: Arc<Flowtrace>) -> Self {
        Self {
            db,
            start_ts: None,
            end_ts: None,
    ...
    fn validate_complexity(&self) -> Result<()> {
        ...
        if let (Some(start), Some(end)) = (self.start_ts, self.end_ts) {
            ...
            {
                return Err(FlowtraceError::Validation(format!(
...
mod tests {
    ...
    #[tokio::test]
    async fn test_flowtrace_basic_operations() {
    ...
    #[tokio::test]
    async fn test_flowtrace_temporal_query() {
        let dir = tempdir().unwrap();
        let db = Flowtrace::open(dir.path()).unwrap();
    ...
    async fn test_query_builder() {
        let dir = tempdir().unwrap();
        let db = Arc::new(Flowtrace::open(dir.path()).unwrap());
