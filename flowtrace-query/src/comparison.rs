...
use anyhow::Result;
use flowtrace_core::eval_dataset::EvalRun;
use parking_lot::RwLock;
use serde::{Deserialize, Serialize};
use std::collections::HashMap;
use thiserror::Error;
// Actually EvalRun is in flowtrace-core.
    ...
    fn extract_metric_values(&self, run: &EvalRun, metric: &str) -> Result<Vec<f64>> {
        ...
        for result in &run.results {
            if result.eval_metrics.contains_key(metric) {
                // EvalMetric is a struct in flowtrace_core::eval
                ...
                // Wait, EvalMetric was shown in `flowtrace-core/src/lib.rs` re-export.
                // Let's read flowtrace-core/src/eval.rs to be sure.
