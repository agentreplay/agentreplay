...
```rust
use flowtrace_index::embedding::{LocalEmbeddingProvider, EmbeddingProvider};
...
```rust
use flowtrace_index::{VamanaConfig, VamanaIndex, PQCodebooks};
...
```bash
# Run all tests
cargo test -p flowtrace-index

# Run embedding tests only
cargo test -p flowtrace-index embedding::

# Run integration tests
cargo test -p flowtrace-index --test vamana_integration_test
```
...
Same as the parent Flowtrace project.
