...
impl ModelManager {
    ...
    fn get_model_dir() -> Result<PathBuf, ModelError> {
        // Linux: ~/.local/share/flowtrace/models/
        // macOS: ~/Library/Application Support/flowtrace/models/
        // Windows: %APPDATA%\flowtrace\models\
        let data_dir = dirs::data_dir().unwrap_or_else(|| PathBuf::from("."));

        Ok(data_dir.join("flowtrace").join("models"))
    }
...
mod tests {
    ...
    fn test_model_dir() {
        ...
        assert!(dir.to_string_lossy().contains("flowtrace"));
