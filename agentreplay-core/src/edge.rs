...
///
/// ```
/// use agentreplay_core::edge::{HybridLogicalClock, HlcTimestamp};
///
...
/// # Examples
/// ```
/// use agentreplay_core::{validate_timestamp, TimestampConfig};
///
...
pub fn validate_timestamp(timestamp_us: u64, config: &TimestampConfig) -> crate::error::Result<()> {
    ...
    if let Some(min) = config.min_timestamp {
        if timestamp_us < min {
            return Err(crate::error::AgentreplayError::InvalidTimestamp(
                format!(
    ...
    if timestamp_us > now && config.enforce_validation {
        ...
        if drift > MAX_FUTURE_DRIFT_US {
            return Err(crate::error::AgentreplayError::InvalidTimestamp(format!(
...
/// # Examples
/// ```
/// use agentreplay_core::checked_timestamp_add;
///
...
pub fn checked_timestamp_add(timestamp_us: u64, duration_us: u64) -> crate::error::Result<u64> {
    timestamp_us.checked_add(duration_us).ok_or_else(|| {
        crate::error::AgentreplayError::InvalidTimestamp(format!(
...
/// # Examples
/// ```
/// use agentreplay_core::checked_timestamp_sub;
///
...
impl AgentFlowEdge {
    ...
    /// share the same Agentreplay instance with complete data isolation.
