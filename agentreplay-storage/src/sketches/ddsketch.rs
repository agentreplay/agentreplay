...
//!
//! **Gap #1 Fix**: Handles subnormal floats and extreme values correctly.
//! Values are clamped to [MIN_INDEXABLE_VALUE, MAX_INDEXABLE_VALUE] to prevent
//! i32 overflow in index calculation from ln() approaching -∞ for subnormals.

use std::collections::BTreeMap;

/// Minimum indexable value - prevents ln(value) from approaching -∞
/// Subnormal floats below this threshold are clamped to this value.
/// Set to 1e-300 which is well above subnormal range (~1e-308) but
/// still supports microsecond-level latency measurements (1e-6 seconds).
const MIN_INDEXABLE_VALUE: f64 = 1e-300;

/// Maximum indexable value - prevents index overflow for extreme values
/// Values above this are clamped. Supports up to ~1e300 which covers
/// all practical measurement ranges (years in microseconds is ~1e22).
...
impl DDSketch {
    ...
    /// # Example
    /// ```
    /// use agentreplay_storage::sketches::DDSketch;
    ...
    ///
    /// **Gap #1 Fix**: Clamps value to safe range to prevent i32 overflow.
    /// Subnormal floats (approaching 1e-308) would cause ln(value) → -∞,
    /// resulting in undefined behavior when cast to i32.
    #[inline]
    fn index(&self, value: f64) -> i32 {
        debug_assert!(value > 0.0);
        // Clamp to safe range to prevent overflow from extreme values
...
mod tests {
    ...
    #[test]
    fn test_subnormal_values_gap1() {
        // Gap #1: Test that subnormal floats don't cause i32 overflow
        let mut sketch = DDSketch::new(0.01);

        // These extreme values would previously cause index() to overflow
        // because ln(1e-320) approaches -∞
        sketch.add(1e-320); // Subnormal (clamped to MIN_INDEXABLE_VALUE)
        sketch.add(1e-300); // At MIN_INDEXABLE_VALUE boundary
        sketch.add(1e300); // At MAX_INDEXABLE_VALUE boundary
        sketch.add(f64::MAX); // Maximum f64 (clamped to MAX_INDEXABLE_VALUE)
        ...
        // Quantile queries should work without panic
        let p50 = sketch.quantile(0.50);
        assert!(p50.is_finite(), "P50 should be finite: {}", p50);

        let p99 = sketch.quantile(0.99);
        assert!(p99.is_finite(), "P99 should be finite: {}", p99);
    }

    #[test]
    fn test_extreme_small_values_gap1() {
        let mut sketch = DDSketch::new(0.01);

        // Add many very small values (sub-microsecond latencies)
        for _ in 0..1000 {
            sketch.add(1e-9); // 1 nanosecond
        }
    ...
    #[test]
    fn test_extreme_large_values_gap1() {
        let mut sketch = DDSketch::new(0.01);

        // Add extreme large values (simulating overflow scenarios)
        for _ in 0..100 {
            sketch.add(1e200); // Very large value
        }
        for _ in 0..100 {
            sketch.add(1e-200); // Very small value
        }

        assert_eq!(sketch.count(), 200);

        // Both extremes should be represented
        assert!(sketch.min() > 0.0);
        assert!(sketch.max() > sketch.min());
    }
}
