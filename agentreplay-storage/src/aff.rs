...
impl AFFHeader {
    ...
    pub fn new(
        ...
        let mut header = AFFHeader {
            magic: *AFF_MAGIC,
            version: AFF_VERSION,
            schema_version: agentreplay_core::AFF_SCHEMA_VERSION,
            _padding1: [0; 3],
            edge_count,
            start_time_us,
            end_time_us,
    ...
    pub fn from_bytes(bytes: &[u8; 256]) -> Result<Self> {
        ...
        if &magic != AFF_MAGIC {
            return Err(AgentreplayError::Corruption(format!(
        ...
        if !header.verify_checksum() {
            return Err(AgentreplayError::Corruption(
...
impl AFFReader {
    ...
    pub fn read_edge(&mut self, index: u64) -> Result<AgentFlowEdge> {
        if index >= self.header.edge_count {
            return Err(AgentreplayError::InvalidArgument(format!(
        ...
        let mut edge_bytes = [0u8; 128];
        self.file.read_exact(&mut edge_bytes)?;
        AgentFlowEdge::from_bytes(&edge_bytes).map_err(AgentreplayError::Io)
    }
