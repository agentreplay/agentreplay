# Flowtrace Plugin SDK for Python

Build Flowtrace plugins in Python and compile them to WASM.

## Installation

```bash
pip install flowtrace-plugin-sdk
```

For building plugins to WASM:

```bash
pip install flowtrace-plugin-sdk[build]
```

## Quick Start

```python
from flowtrace_plugin import Evaluator, TraceContext, EvalResult, PluginMetadata, export
...
```bash
flowtrace-plugin-build src/evaluator.py -o my_plugin.wasm
```

Or using componentize-py directly:

```bash
componentize-py -d wit -w flowtrace-plugin componentize src/evaluator.py -o my_plugin.wasm
```

## Using Host Functions

```python
from flowtrace_plugin import Host
