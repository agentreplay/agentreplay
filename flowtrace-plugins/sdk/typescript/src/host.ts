...
export const Host = {
        ...
        : level === 'debug'
          ...
          : level === 'info'
            ...
            : level === 'warn'
              ? 3
              : 4;
    (globalThis as any).__flowtrace_log?.(levelNum, message);
  },
  ...
  getConfig<T = Record<string, unknown>>(): T {
    const configJson = (globalThis as any).__flowtrace_get_config?.() ?? '{}';
  ...
  getConfigValue(key: string): string | null {
    return (globalThis as any).__flowtrace_get_config_value?.(key) ?? null;
  },
  ...
  queryTraces(filterJson: string, limit: number = 100): TraceContext[] {
    const resultJson =
      (globalThis as any).__flowtrace_query_traces?.(filterJson, limit) ?? '[]';
  ...
  getTrace(traceId: TraceId): TraceContext | null {
    ...
    const resultJson = (globalThis as any).__flowtrace_get_trace?.(idStr);
  ...
  async httpRequest(
    ...
    const result = await (globalThis as any).__flowtrace_http_request?.(
      method,
      url,
  ...
  embedText(text: string): Embedding {
    const resultJson =
      (globalThis as any).__flowtrace_embed_text?.(text) ?? '[]';
  ...
  embedBatch(texts: string[]): Embedding[] {
    const resultJson =
      (globalThis as any).__flowtrace_embed_batch?.(JSON.stringify(texts)) ??
      '[]';
  ...
  getEnv(name: string): string | null {
    return (globalThis as any).__flowtrace_get_env?.(name) ?? null;
  },
};
