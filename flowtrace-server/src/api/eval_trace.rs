...
pub fn build_eval_trace_v1(state: &AppState, root_edge: &flowtrace_core::AgentFlowEdge) -> EvalTraceV1 {
    let mut eval_trace = EvalTraceV1::new(format!("0x{:x}", root_edge.edge_id), root_edge.session_id);

    let mut edges: Vec<flowtrace_core::AgentFlowEdge> = state
        .db
    ...
    for edge in &edges {
        ...
        if let Some(payload) = payload.as_ref() {
            if payload.error_type.is_some() || edge.get_span_type() == flowtrace_core::SpanType::Error {
                status = "error".to_string();
            }
        } else if edge.get_span_type() == flowtrace_core::SpanType::Error {
            status = "error".to_string();
        }
    ...
    eval_trace.trace_ref = Some(flowtrace_core::TraceRefV1 {
        schema_version: flowtrace_core::EVAL_TRACE_SCHEMA_VERSION_V1.to_string(),
