...
// flowtrace-server/src/api/ingest.rs
//! Ingestion API for Flowtrace traces
//!
//! Accepts FlowtraceSpan batches via POST and converts them to AgentFlowEdge records.
//!
//! ## Architecture
//!
...
use flowtrace_core::{AgentFlowEdge, Environment, SpanType};
...
fn validate_span(idx: usize, span: &FlowtraceSpan) -> Result<(), String> {
...
fn sanitize_span(span: &FlowtraceSpan) -> Result<FlowtraceSpan, String> {
    let validated_name =
...
async fn ingest_direct(
    ...
    for (idx, span) in request.spans.iter().enumerate() {
        ...
        let validated_span = FlowtraceSpan {
            span_id: span.span_id.clone(),
            trace_id: span.trace_id.clone(),
            parent_span_id: span.parent_span_id.clone(),
            name: validated_name,
...
/// Convert FlowtraceSpan to AgentFlowEdge
...
/// and converts it to Flowtrace's internal AgentFlowEdge format
