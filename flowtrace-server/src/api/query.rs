...
use flowtrace_core::{checked_timestamp_add, AgentFlowEdge};
...
pub struct AppState {
    ...
    pub db_path: String,
    pub saved_view_registry: Arc<RwLock<flowtrace_core::SavedViewRegistry>>,
    ...
    pub vector_index: Option<Arc<flowtrace_index::HnswIndex>>,
...
impl From<AgentFlowEdge> for TraceView {
    fn from(edge: AgentFlowEdge) -> Self {
        ...
        let status = if edge.get_span_type() == flowtrace_core::SpanType::Error {
    ...
    let query_future = async {
        ...
        edges.retain(|e| {
            ...
            if let Some(ref statuses) = params.status {
                let status = if e.get_span_type() == flowtrace_core::SpanType::Error {
                    "error"
                } else {
                    "completed"
                };
            ...
            if let Some(has_err) = params.has_errors {
                let is_err = e.get_span_type() == flowtrace_core::SpanType::Error;
                if has_err != is_err {
                    return false;
                }
            }
...
pub async fn find_edge_by_id_or_session(
    ...
    let scan_db_for_session = |db: &Arc<Flowtrace>| -> Option<AgentFlowEdge> {
