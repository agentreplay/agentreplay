...
pub struct StorageConfig {
    /// Path to Flowtrace data directory
...
impl ServerConfig {
    ...
    /// - FLOWTRACE_HTTP_ADDR: HTTP listen address (default: 127.0.0.1:9600)
    /// - FLOWTRACE_DATA_DIR: Data directory path (default: ./flowtrace-data)
    /// - FLOWTRACE_AUTH_ENABLED: Enable authentication (default: false)
    /// - FLOWTRACE_JWT_SECRET: JWT secret for token validation
    /// - FLOWTRACE_API_KEYS: Comma-separated API keys (format: key:tenant_id)
    /// - FLOWTRACE_MAX_CONNECTIONS: Max concurrent connections (default: 1000)
    /// - FLOWTRACE_REQUEST_TIMEOUT: Request timeout in seconds (default: 30)
    /// - FLOWTRACE_ENABLE_CORS: Enable CORS (default: true)
    pub fn from_env() -> Self {
        let mut config = Self::default();

        // Server configuration
        if let Ok(addr) = std::env::var("FLOWTRACE_HTTP_ADDR") {
            config.server.listen_addr = addr;
        }

        if let Ok(max_conn) = std::env::var("FLOWTRACE_MAX_CONNECTIONS") {
            if let Ok(val) = max_conn.parse() {
                config.server.max_connections = val;
            }
        }

        if let Ok(timeout) = std::env::var("FLOWTRACE_REQUEST_TIMEOUT") {
            if let Ok(val) = timeout.parse() {
                config.server.request_timeout_secs = val;
            }
        }

        if let Ok(cors) = std::env::var("FLOWTRACE_ENABLE_CORS") {
            config.server.enable_cors = cors.parse().unwrap_or(true);
        }

        // Storage configuration
        if let Ok(data_dir) = std::env::var("FLOWTRACE_DATA_DIR") {
            config.storage.data_dir = PathBuf::from(data_dir);
        }

        if let Ok(compress) = std::env::var("FLOWTRACE_ENABLE_COMPRESSION") {
            config.storage.enable_compression = compress.parse().unwrap_or(true);
        }

        if let Ok(use_projects) = std::env::var("FLOWTRACE_USE_PROJECT_STORAGE") {
            config.storage.use_project_storage = use_projects.parse().unwrap_or(false);
        }

        // Auth configuration
        if let Ok(enabled) = std::env::var("FLOWTRACE_AUTH_ENABLED") {
            config.auth.enabled = enabled.parse().unwrap_or(false);
        }

        if let Ok(secret) = std::env::var("FLOWTRACE_JWT_SECRET") {
            config.auth.jwt_secret = Some(secret);
        }

        if let Ok(keys) = std::env::var("FLOWTRACE_API_KEYS") {
...
mod tests {
    ...
    #[test]
    fn test_from_env() {
        std::env::set_var("FLOWTRACE_HTTP_ADDR", "0.0.0.0:8080");
        std::env::set_var("FLOWTRACE_AUTH_ENABLED", "true");

        let config = ServerConfig::from_env();
        assert_eq!(config.server.listen_addr, "0.0.0.0:8080");
        assert!(config.auth.enabled);

        std::env::remove_var("FLOWTRACE_HTTP_ADDR");
        std::env::remove_var("FLOWTRACE_AUTH_ENABLED");
    }
}
