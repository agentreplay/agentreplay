...
pub async fn run_server(config: ServerConfig) -> Result<()> {
    ...
    tracing::info!("Starting Flowtrace Server");
    ...
    let project_manager = if use_project_storage {
        ...
        tracing::info!("Using single database mode (set FLOWTRACE_USE_PROJECT_STORAGE=true for per-project storage)");
        None
    };
    ...
    // Open Flowtrace database (fallback for non-project mode or legacy queries)
    ...
    let db = if config.storage.high_performance {
        ...
        Arc::new(Flowtrace::open_high_performance(&config.storage.data_dir)?)
    } else {
        tracing::info!("Using standard WAL mode (Segmented)");
        Arc::new(Flowtrace::open(&config.storage.data_dir)?)
    };
    ...
    let saved_view_registry = Arc::new(tokio::sync::RwLock::new(
        flowtrace_core::SavedViewRegistry::new(&config.storage.data_dir),
    ));
    ...
    let vector_index = {
        let hnsw_config = flowtrace_index::HnswConfig {
            max_connections: 16,
            max_connections_layer0: 32,
            level_multiplier: 1.0 / (16.0_f32).ln(),
            ef_construction: 200,
            ef_search: 50,
            metric: flowtrace_index::HnswDistanceMetric::Cosine,
        ...
        let index = flowtrace_index::HnswIndex::new(384, hnsw_config);
    ...
    let authenticator: Arc<dyn Authenticator> = if config.auth.enabled {
        ...
        let allow_noauth = std::env::var("FLOWTRACE_ALLOW_NOAUTH")
            .unwrap_or_default()
            .to_lowercase();
        ...
        if !is_explicitly_allowed && !is_localhost {
            tracing::error!(
                ...
                ║    2. Set: export FLOWTRACE_ALLOW_NOAUTH=true               ║\n\
                ║                                                               ║\n\
            ...
            anyhow::bail!(
                ...
                Bind to localhost, set FLOWTRACE_ALLOW_NOAUTH=true, or enable authentication.",
                config.server.listen_addr
            );
        }
    ...
    let authed_routes = Router::new()
        ...
        .nest("/api/v1/git", {
            let git_state = Arc::new(api::GitVersioningState::new("Flowtrace User"));
