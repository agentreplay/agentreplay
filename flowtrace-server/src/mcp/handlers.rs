                ...
                match self.state.db.query_temporal_range(start, now) {
                    Ok(edges) => {
                        let errors: Vec<serde_json::Value> = edges
                            .iter()
                            .filter(|e| {
                                matches!(e.get_span_type(), flowtrace_core::SpanType::Error)
                            })
                            .take(50)
                            .map(|e| {
                                json!({
