...
```rust
use agentreplay_index::{VamanaConfig, VamanaIndex, PQCodebooks};
...
```bash
# Run all tests
cargo test -p agentreplay-index

# Run embedding tests only
cargo test -p agentreplay-index embedding::

# Run integration tests
cargo test -p agentreplay-index --test vamana_integration_test
```
