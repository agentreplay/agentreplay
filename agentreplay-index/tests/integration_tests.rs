...
//! Real-World Integration Tests for Agentreplay Index
//!
...
use agentreplay_index::hnsw::{HnswConfig, HnswIndex};
use agentreplay_index::vector_quantized::Precision;
...
fn test_incremental_building() {
    ...
    let stats = index.stats();
    // Estimate memory: ~(dim * 4 bytes per vector) + (~64 bytes per neighbor entry * avg_connections)
    let estimated_bytes = stats.num_vectors * stats.dimension * 4 + 
                          (stats.num_vectors as f32 * stats.avg_connections * 64.0) as usize;
    println!(
        "✓ Final: {} vectors, {:.2} MB",
        stats.num_vectors,
