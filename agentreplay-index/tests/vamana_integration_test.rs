...
use agentreplay_index::product_quantization::{PQCodebooks, PQCodes};
use agentreplay_index::vamana::{VamanaConfig, VamanaIndex};
use ndarray::Array1;
use rand::Rng;
...
fn test_vamana_large_scale_simulation() {
    ...
    for &n in &scales {
        let f32_gb = (n as u64 * dim as u64 * 4) as f64 / (1024.0 * 1024.0 * 1024.0);
        let f16_gb = (n as u64 * dim as u64 * 2) as f64 / (1024.0 * 1024.0 * 1024.0);
        let pq_gb = (n as u64 * 48) as f64 / (1024.0 * 1024.0 * 1024.0); // 48 bytes for 384-dim
