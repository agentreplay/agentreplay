...
lazy_static! {
    ...
    // Use unwrap_or_else to handle already-registered metrics (happens in parallel tests)
    pub static ref INSERT_COUNT: Counter = register_counter!(
        "hnsw_insert_total",
        "Total number of vector insertions"
    )
    .unwrap_or_else(|_| Counter::new("hnsw_insert_total", "Total number of vector insertions").unwrap());

    pub static ref SEARCH_COUNT: Counter = register_counter!(
        "hnsw_search_total",
        "Total number of searches performed"
    )
    .unwrap_or_else(|_| Counter::new("hnsw_search_total", "Total number of searches performed").unwrap());

    pub static ref ERROR_COUNT: Counter = register_counter!(
        "hnsw_errors_total",
        "Total number of errors encountered"
    )
    .unwrap_or_else(|_| Counter::new("hnsw_errors_total", "Total number of errors encountered").unwrap());
    ...
    .unwrap_or_else(|_| {
        Histogram::with_opts(
            prometheus::HistogramOpts::new("hnsw_search_duration_seconds", "Search latency in seconds")
    ...
    .unwrap_or_else(|_| {
        Histogram::with_opts(
            prometheus::HistogramOpts::new("hnsw_insert_duration_seconds", "Insert latency in seconds")
    ...
    .unwrap_or_else(|_| {
        Histogram::with_opts(
            prometheus::HistogramOpts::new("hnsw_distance_calculations_per_search", "Number of distance calculations per search")
    ...
    .unwrap_or_else(|_| {
        Histogram::with_opts(
            prometheus::HistogramOpts::new("hnsw_search_results_returned", "Number of results returned per search")
    ...
    .unwrap_or_else(|_| {
        HistogramVec::new(
            prometheus::HistogramOpts::new("hnsw_operation_duration_seconds", "Operation latency in seconds by operation type")
    ...
    .unwrap_or_else(|_| Gauge::new("hnsw_nodes_total", "Current number of nodes in the index").unwrap());

    pub static ref MEMORY_BYTES: Gauge = register_gauge!(
        "hnsw_memory_bytes",
        "Estimated memory usage in bytes"
    )
    .unwrap_or_else(|_| Gauge::new("hnsw_memory_bytes", "Estimated memory usage in bytes").unwrap());

    pub static ref AVG_NEIGHBORS: Gauge = register_gauge!(
        "hnsw_avg_neighbors",
        "Average number of neighbors per node"
    )
    .unwrap_or_else(|_| Gauge::new("hnsw_avg_neighbors", "Average number of neighbors per node").unwrap());

    pub static ref MAX_LAYER: Gauge = register_gauge!(
        "hnsw_max_layer",
        "Maximum layer in the index"
    )
    .unwrap_or_else(|_| Gauge::new("hnsw_max_layer", "Maximum layer in the index").unwrap());

    pub static ref VECTOR_DIMENSION: Gauge = register_gauge!(
        "hnsw_vector_dimension",
        "Dimension of vectors in the index"
    )
    .unwrap_or_else(|_| Gauge::new("hnsw_vector_dimension", "Dimension of vectors in the index").unwrap());
}
