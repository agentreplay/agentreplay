...
use sochdb_index::product_quantization::{DistanceTable, PQCodebooks, PQCodes};
use sochdb_index::vector_storage::VectorStorage;
use dashmap::DashMap;
use ndarray::Array1;
...
// ============================================================================
// Gap #6: Lazy Backedge Update Configuration
// ============================================================================

/// Default threshold for automatic backedge flush (Gap #6)
const DEFAULT_BACKEDGE_FLUSH_THRESHOLD: usize = 10000;

/// Backedge delta statistics (Gap #6)
#[derive(Debug, Default, Clone)]
pub struct BackedgeDeltaStats {
    /// Current number of pending backedge entries
    pub pending_entries: usize,
    /// Total nodes with pending backedges
    pub pending_nodes: usize,
    /// Total backedges flushed since creation
...
pub struct VamanaConfig {
    ...
    pub pq_subdim: usize,
    /// Gap #6: Threshold for automatic backedge flush
    pub backedge_flush_threshold: usize,
}
...
impl Default for VamanaConfig {
    fn default() -> Self {
        Self {
            ...
            pq_subspaces: 48,
            pq_subdim: 8,
            backedge_flush_threshold: DEFAULT_BACKEDGE_FLUSH_THRESHOLD,
        }
    }
}
...
impl VamanaIndex {
    ...
    /// Get backedge delta statistics (Gap #6)
    ///
    /// Returns current state of lazy backedge updates for monitoring.
    pub fn backedge_stats(&self) -> BackedgeDeltaStats {
        let mut pending_entries = 0;
        let pending_nodes = self.backedge_deltas.len();

        for entry in self.backedge_deltas.iter() {
            pending_entries += entry.value().len();
        }

        BackedgeDeltaStats {
            pending_entries,
            pending_nodes,
            total_flushed: 0, // Would need to track separately
            flush_count: 0,   // Would need to track separately
        }
    }

    /// Check if backedge flush is recommended (Gap #6)
    ///
    /// Returns true if pending backedges exceed threshold.
    pub fn should_flush_backedges(&self) -> bool {
        let stats = self.backedge_stats();
        stats.pending_entries > self.config.backedge_flush_threshold
    }

    /// Force backedge flush if threshold exceeded (Gap #6)
    ///
    /// Call periodically during bulk ingestion to prevent memory growth.
    pub fn maybe_flush_backedges(&self) {
        if self.should_flush_backedges() {
            self.consolidate_backedges();
        }
    }
    ...
    pub fn save<P: AsRef<Path>>(&self, dir: P) -> io::Result<()> {
        ...
        bincode::serialize_into(encoder, &pq_data)
            .map_err(|e| io::Error::new(io::ErrorKind::Other, e.to_string()))?;
        ...
        bincode::serialize_into(encoder, &metadata)
            .map_err(|e| io::Error::new(io::ErrorKind::Other, e.to_string()))?;
    ...
    pub fn load<P: AsRef<Path>>(dir: P) -> io::Result<Self> {
        ...
        let metadata: VamanaMetadata = bincode::deserialize_from(decoder)
            .map_err(|e| io::Error::new(io::ErrorKind::Other, e.to_string()))?;
        ...
        let graph_data: Vec<(u128, VamanaNode)> = bincode::deserialize_from(decoder)
            .map_err(|e| io::Error::new(io::ErrorKind::Other, e.to_string()))?;
        ...
        let pq_data: Vec<(u128, PQCodes)> = bincode::deserialize_from(decoder)
            .map_err(|e| io::Error::new(io::ErrorKind::Other, e.to_string()))?;
...
mod tests {
    ...
    fn generate_random_vectors(n: usize, dim: usize) -> Vec<Array1<f32>> {
        ...
        (0..n)
            .map(|_| Array1::from_iter((0..dim).map(|_| rng.gen::<f32>())))
            .collect()
    }
    ...
    #[test]
    #[ignore] // Slow test - run manually with: cargo test test_vamana_memory_efficiency -- --ignored
