...
impl ModelManager {
    ...
    fn get_model_dir() -> Result<PathBuf, ModelError> {
        // Linux: ~/.local/share/agentreplay/models/
        // macOS: ~/Library/Application Support/agentreplay/models/
        // Windows: %APPDATA%\agentreplay\models\
        let data_dir = dirs::data_dir().unwrap_or_else(|| PathBuf::from("."));

        Ok(data_dir.join("agentreplay").join("models"))
    }
...
mod tests {
    ...
    fn test_model_dir() {
        ...
        assert!(dir.to_string_lossy().contains("agentreplay"));
