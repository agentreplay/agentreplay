...
const SDKS: Record<SdkType, {
  ...
  python: {
    label: 'Python',
    installCmd: 'pip install flowtrace-client && flowtrace-install',
    installLabel: 'Install via pip',
    verifyCode: (projectId) => `import os
from flowtrace_client import FlowtraceClient

# Initialize client
client = FlowtraceClient(
    url=os.getenv("FLOWTRACE_URL", "http://127.0.0.1:9600"),
    tenant_id=int(os.getenv("FLOWTRACE_TENANT_ID", "1")),
    project_id=int(os.getenv("FLOWTRACE_PROJECT_ID", "${projectId}")),
)

# Create a test trace
trace = client.create_trace(
    agent_id=1,
    payload={"test": "Hello Flowtrace!"}
)
...
print(f"🚀 Visit http://localhost:5173/traces to see your data")`
  ...
  typescript: {
    label: 'TypeScript',
    installCmd: 'npm install flowtrace-client',
    installLabel: 'Install via npm',
    verifyCode: (projectId) => `import { FlowtraceClient } from 'flowtrace-client';

// Initialize client
const client = new FlowtraceClient({
  url: process.env.FLOWTRACE_URL || 'http://127.0.0.1:9600',
  tenantId: parseInt(process.env.FLOWTRACE_TENANT_ID || '1'),
  projectId: parseInt(process.env.FLOWTRACE_PROJECT_ID || '${projectId}'),
});
...
console.log(\`🚀 Visit http://localhost:5173/traces to see your data\`);`
  ...
  go: {
    label: 'Go',
    installCmd: 'go get github.com/sushanthpy/flowtrace-go',
...
import (
	...
	"github.com/sushanthpy/flowtrace-go"
)

func main() {
	// Initialize client
	client := flowtrace.NewClient(
		os.Getenv("FLOWTRACE_URL"),
		getEnvInt("FLOWTRACE_TENANT_ID", 1),
		getEnvInt("FLOWTRACE_PROJECT_ID", ${projectId}),
	)

	// Create a test trace
	trace, _ := client.CreateTrace(1, map[string]interface{}{
		"test": "Hello Flowtrace!",
	})
...
func getEnvInt(key string, defaultVal int) int {
  ...
  rust: {
    label: 'Rust',
    installCmd: 'cargo add flowtrace-client',
    installLabel: 'Install via cargo',
    verifyCode: (projectId) => `use flowtrace_client::FlowtraceClient;
use std::env;

#[tokio::main]
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    // Initialize client
    let client = FlowtraceClient::new(
        env::var("FLOWTRACE_URL").unwrap_or("http://127.0.0.1:9600".to_string()),
        env::var("FLOWTRACE_TENANT_ID").unwrap_or("1".to_string()).parse()?,
        env::var("FLOWTRACE_PROJECT_ID").unwrap_or("${projectId}".to_string()).parse()?,
    );

    // Create a test trace
    let trace = client.create_trace(
        1,
        serde_json::json!({"test": "Hello Flowtrace!"})
    ).await?;
...
export default function SetupWizard({ onComplete }: SetupWizardProps) {
      ...
      for (let attempt = 0; attempt < 3; attempt++) {
        try {
          ...
          const createdProjectData = {
            ...
            created_at: Date.now(),
            env_vars: {
              FLOWTRACE_URL: data.env_vars.flowtrace_url,
              FLOWTRACE_TENANT_ID: data.env_vars.tenant_id,
              FLOWTRACE_PROJECT_ID: data.env_vars.project_id,
            }
          };
  ...
  const completeSetup = () => {
    ...
    localStorage.setItem('flowtrace_setup_complete', 'true');
    localStorage.setItem('flowtrace_default_project', createdProject.project_id.toString());
          ...
          >
            ...
            {currentStep === 0 && (
              <div>
                ...
                <p className="text-textSecondary mb-6">
                  Select your preferred language and install the Flowtrace SDK to start tracking your LLM applications.
                </p>
            ...
            {currentStep === 2 && createdProject && (
              <div>
                ...
                <p className="text-textSecondary mb-6">
                  Add these environment variables to your project to connect to FlowTrace.
                </p>
            ...
            {currentStep === 3 && createdProject && (
              <div>
                ...
                <p className="text-textSecondary mb-6">
                  Run this example code to verify your FlowTrace setup is working correctly.
                </p>
