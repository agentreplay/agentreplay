  ...
  return (
    <div className="flex flex-col h-full bg-background rounded-2xl border border-border overflow-hidden">
      ...
      >
        ...
        {isPlaying && currentIndex < messages.length && visibleMessages.length > 0 && (
          <div className="flex items-center gap-2 ml-8 text-textTertiary animate-pulse">
            <div className="flex gap-1">
              <span className="w-2 h-2 bg-textTertiary rounded-full animate-bounce" style={{ animationDelay: '0ms' }} />
              <span className="w-2 h-2 bg-textTertiary rounded-full animate-bounce" style={{ animationDelay: '150ms' }} />
              <span className="w-2 h-2 bg-textTertiary rounded-full animate-bounce" style={{ animationDelay: '300ms' }} />
            </div>
            <span className="text-xs">
              {messages[currentIndex]?.role === 'user' ? 'User typing...' : 'Assistant responding...'}
            </span>
          </div>
        )}
      </div>
      ...
      {/* Footer with stats */}
      <div className="px-4 py-2 bg-surface-elevated border-t border-border flex items-center justify-between text-xs text-textTertiary">
        <div className="flex items-center gap-4">
          <span className="flex items-center gap-1">
            <Clock className="w-3 h-3" />
            {visibleMessages.length > 0 && messages[0] 
              ? formatDuration(
                  visibleMessages[visibleMessages.length - 1]?.timestamp - messages[0].timestamp
                )
              : '0:00'
            }
          </span>
          <span className="flex items-center gap-1">
            ...
            {visibleMessages.filter(m => m.role === 'user').length} user / {visibleMessages.filter(m => m.role === 'assistant').length} assistant
          </span>
        </div>
        <span>
          Total tokens: {visibleMessages.reduce((sum, m) => sum + (m.tokens || 0), 0).toLocaleString()}
        </span>
      </div>
    </div>
  );
...
// CSS animation for slide-in effect (add to globals.css if needed)
// .animate-slide-in {
