...
import { flowtraceClient, Agent } from '../lib/flowtrace-api';
...
export default function Agents() {
    ...
    const { nodes, links } = useMemo(() => {
        ...
        // Central hub (FlowTrace Server)
        nodes.push({
            id: 'hub',
            type: 'service',
            label: 'FlowTrace Core',
            x: 400,
            y: 300,
            status: 'active'
        });
                    ...
                    <div className="absolute top-4 right-4 w-80 bg-surface border border-border rounded-lg shadow-xl p-4 animate-in slide-in-from-right-10">
                        ...
                        <div className="space-y-4">
                            <div>
                                ...
                                <div className={`inline-flex items-center gap-2 px-2 py-1 rounded-full text-xs font-medium ${selectedNode.status === 'active'
                                        ? 'bg-success-bg text-success border border-success/20'
                                        : 'bg-surface-hover text-textSecondary border border-border'
                                    }`}>
                                    <div className={`w-1.5 h-1.5 rounded-full ${selectedNode.status === 'active' ? 'bg-success' : 'bg-textSecondary'}`} />
                                    <span className="capitalize">{selectedNode.status}</span>
                                </div>
                            </div>

                            <div>
                                <div className="text-xs text-textSecondary uppercase tracking-wider mb-1">Type</div>
                                <div className="text-sm text-textPrimary capitalize">{selectedNode.type === 'external' ? 'LLM Model' : selectedNode.type}</div>
                            </div>

                            {/* Show call count and latency for AI-analyzed nodes */}
                            {selectedNode.calls !== undefined && (
                                <div>
                                    <div className="text-xs text-textSecondary uppercase tracking-wider mb-1">Total Calls</div>
                                    <div className="text-sm text-textPrimary font-medium">{selectedNode.calls.toLocaleString()}</div>
                                </div>
                            )}
                            
                            {selectedNode.avgLatency !== undefined && selectedNode.avgLatency > 0 && (
                                <div>
                                    ...
                                    <div className="text-sm text-textPrimary font-medium">
                                        {selectedNode.avgLatency >= 1000 
                                            ? `${(selectedNode.avgLatency / 1000).toFixed(2)}s`
                            ...
                            {selectedNode.metadata && (
                                <>
                                    <div>
                                        <div className="text-xs text-textSecondary uppercase tracking-wider mb-1">Agent ID</div>
                                        <code className="text-xs bg-surface-hover px-1.5 py-0.5 rounded text-textPrimary break-all">
                                            {selectedNode.metadata.agent_id}
                                        </code>
                                    </div>
                                    {selectedNode.metadata.last_seen && (
                                        <div>
                                            <div className="text-xs text-textSecondary uppercase tracking-wider mb-1">Last Seen</div>
                                            <div className="text-sm text-textPrimary">
                                                {new Date(selectedNode.metadata.last_seen).toLocaleString()}
                                            </div>
                                        </div>
                                    )}
                                </>
