...
export default function TraceDetailsPage() {
  ...
  const fetchTraceDetails = async (id: string) => {
    ...
    try {
      ...
      const details: any = await flowtraceClient.getTrace(
        id,
      ...
      try {
        const obs = await flowtraceClient.getTraceObservations(
          id,
          tenantId ? parseInt(tenantId) : 1,
          projectId ? parseInt(projectId) : 0
        );
  ...
  const confirmDelete = async () => {
    ...
    try {
      if (traceId) {
        await flowtraceClient.deleteTrace(traceId, Number(projectId || 0));
