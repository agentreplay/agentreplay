...
export default function Traces() {
  ...
  const handleDeleteProject = async () => {
    ...
    setIsDeleting(true);
    try {
      await flowtraceClient.deleteProject(effectiveProjectId);
      ...
      const response = await flowtraceClient.listProjects();
  ...
  const fetchPage = useCallback(
    async (page: number = 1, reset = false) => {
      ...
      try {
        ...
        const response = await flowtraceClient.listTraces({
          limit: PAGE_SIZE,
          offset: offset,
