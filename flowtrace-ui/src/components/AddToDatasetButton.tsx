...
import { flowtraceClient, EvalDataset } from '../lib/flowtrace-api';
...
export function AddToDatasetButton({ traceId, traceMetadata, className = '' }: AddToDatasetButtonProps) {
  ...
  const handleAddToDataset = async (datasetId: string) => {
    ...
    try {
      ...
      await flowtraceClient.addExamples(datasetId, [{
  ...
  const handleCreateAndAdd = async () => {
    ...
    try {
      // Create new dataset
      const response = await flowtraceClient.createDataset(
        newDatasetName.trim(),
