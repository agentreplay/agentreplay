    ...
    const connect = () => {
      ...
      try {
        ...
        ws.onopen = () => {
          ...
          console.log('[Flowtrace] Metrics WebSocket connected');
          setConnected(true);
          setError(null);
        ...
        ws.onmessage = (event) => {
          try {
            ...
            console.error('[Flowtrace] Failed to parse metric update', err);
          }
        };

        ws.onerror = (evt) => {
          console.error('[Flowtrace] Metrics WebSocket error', evt);
          setConnected(false);
        ...
        ws.onclose = (evt) => {
          console.log('[Flowtrace] Metrics WebSocket closed', evt.code, evt.reason);
          setConnected(false);
          setConnectionState('disconnected');
          ...
          if (!cancelled && !evt.wasClean) {
            ...
            console.log(`[Flowtrace] Metrics reconnecting in ${Math.round(currentBackoffRef.current)}ms`);
            reconnectTimeoutRef.current = setTimeout(connect, currentBackoffRef.current);
          }
        };
      } catch (err) {
        console.error('[Flowtrace] Failed to establish metrics WebSocket', err);
