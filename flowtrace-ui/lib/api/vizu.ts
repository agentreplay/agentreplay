...
/**
 * Flowtrace API Client for Tauri Desktop
 * 
...
export const flowtraceClient = new FlowtraceClient();
...
export const fetcher = async (url: string) => {
  ...
  if (url.includes('/costs')) {
    return flowtraceClient.getCosts();
  }
  if (url.includes('/timeseries')) {
    const params = new URL(url, 'http://localhost').searchParams;
    return flowtraceClient.getTimeseries({
