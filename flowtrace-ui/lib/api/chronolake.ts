...
// Flowtrace API client for UI
import axios from 'axios';

const FLOWTRACE_BASE_URL = process.env.NEXT_PUBLIC_FLOWTRACE_URL || 'http://localhost:9600';
// Alias for backwards compatibility
const CHRONOLAKE_BASE_URL = FLOWTRACE_BASE_URL;
...
class ChronoLakeClient {
  ...
  constructor(baseURL: string = CHRONOLAKE_BASE_URL) {
    ...
    this.apiKey = process.env.NEXT_PUBLIC_FLOWTRACE_API_KEY;
    this.circuitBreaker = new CircuitBreaker();
    
    // Load tenant/project from localStorage if available
    if (typeof window !== 'undefined') {
      this.tenantId = localStorage.getItem('flowtrace_tenant_id') || undefined;
      this.projectId = localStorage.getItem('flowtrace_project_id') || undefined;
    }
  }
  ...
  setTenant(tenantId: string) {
    ...
    if (typeof window !== 'undefined') {
      localStorage.setItem('flowtrace_tenant_id', tenantId);
    }
  }
  ...
  setProject(projectId: string) {
    ...
    if (typeof window !== 'undefined') {
      localStorage.setItem('flowtrace_project_id', projectId);
    }
  }
  ...
  private getHeaders() {
    ...
    if (this.apiKey) {
      headers['X-Flowtrace-API-Key'] = this.apiKey;
    }
    ...
    try {
      return await this.circuitBreaker.execute(async () => {
        return await fetchWithRetry(async () => {
          ...
          // Log for debugging
          if (process.env.NODE_ENV === 'development') {
            console.log('[Flowtrace] Fetched trace:', response.data.span_id, 'metadata keys:', Object.keys(response.data.metadata || {}));
          }
...
export const flowtraceClient = new ChronoLakeClient();
// Alias for backwards compatibility
export const FlowtraceClient = ChronoLakeClient;
