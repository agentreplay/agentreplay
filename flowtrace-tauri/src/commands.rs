...
where
    F: FnOnce() -> flowtrace_core::Result<T> + Send + 'static,
    T: Send + 'static,
{
...
impl EmbeddingProvider for ApiEmbeddingProvider {
    fn embed(&self, text: &str) -> Result<Vec<f32>, flowtrace_index::embedding::EmbeddingError> {
        // Use tokio's Handle to run async code in sync context
        let runtime = tokio::runtime::Handle::try_current()
            .map_err(|e| flowtrace_index::embedding::EmbeddingError::InferenceFailed(
        ...
        .map_err(|e| flowtrace_index::embedding::EmbeddingError::InferenceFailed(e))
    }

    fn embed_batch(&self, texts: &[&str]) -> Result<Vec<Vec<f32>>, flowtrace_index::embedding::EmbeddingError> {
...
pub async fn get_trace_stats(
    ...
    let total_observations = edges
        .iter()
        .filter(|e| e.get_span_type() != flowtrace_core::SpanType::Root)
        .count();
...
pub async fn get_timeseries(
    ...
    let mut buckets: std::collections::HashMap<u64, Vec<&flowtrace_core::AgentFlowEdge>> =
...
pub async fn restore_backup(
    ...
    // Hot database reload without restart is complex due to Arc<Flowtrace> in managed state
...
pub async fn run_evaluation(
    ...
    // Build TraceContext
    let trace_context = flowtrace_evals::TraceContext {
        trace_id: edge_id,
        edges,
        input,
        output,
        ...
        for (id, result) in eval_results {
            results.push(EvaluationResult {
                ...
                score: result.metrics.get("score")
                    .and_then(|v| match v { flowtrace_evals::MetricValue::Float(f) => Some(*f), _ => None })
        ...
        if eval_type == "output_quality" || eval_type == "rag_quality" {
            ...
            if llm_client_guard.is_configured() {
                ...
                if trace_context.input.is_some() && trace_context.output.is_some() {
                    ...
                    let geval = flowtrace_evals::evaluators::GEval::new(adapter);
                        ...
                        Ok(eval_result) => {
                            results.push(EvaluationResult {
                                ...
                                score: eval_result.metrics.get("score")
                                    .and_then(|v| match v { 
                                        flowtrace_evals::MetricValue::Float(f) => Some(*f), 
                                        _ => None 
                                    })
    ...
    let metric_entries: Vec<flowtrace_storage::EvalMetricEntry> = results.iter().map(|r| {
        flowtrace_storage::EvalMetricEntry {
            edge_id: edge_id,
    ...
    if !metric_entries.is_empty() {
        ...
        let summary = flowtrace_storage::EvalSummary {
            trace_id: edge_id,
...
fn extract_trace_io(
    db: &Arc<flowtrace_query::Flowtrace>,
    edges: &[flowtrace_core::AgentFlowEdge],
            ...
            if let Ok(payload) = serde_json::from_slice::<serde_json::Value>(&payload_bytes) {
                ...
                if output.is_none() {
                    ...
                    // Direct content/response fields
                        ...
                        if edge.get_span_type() != flowtrace_core::SpanType::ToolResponse {
                ...
                match edge.get_span_type() {
                    flowtrace_core::SpanType::ToolResponse | flowtrace_core::SpanType::Retrieval => {
...
fn get_settings_path(scope: &str, project_path: Option<String>) -> Result<PathBuf, String> {
    match scope {
        "user" => {
            ...
            Ok(home.join(".flowtrace").join("settings.json"))
        }
        "project" => {
            let path = project_path.ok_or("Project path required for project scope")?;
            Ok(PathBuf::from(path).join(".flowtrace").join("settings.json"))
        }
        "local" => {
...
fn get_default_settings() -> FlowtraceSettings {
    FlowtraceSettings {
...
#[tauri::command]
pub async fn get_flowtrace_settings(
    scope: String,
    project_path: Option<String>,
) -> Result<FlowtraceSettings, String> {
...
#[tauri::command]
pub async fn save_flowtrace_settings(
    scope: String,
    settings: FlowtraceSettings,
    project_path: Option<String>,
    state: State<'_, AppState>,
...
use flowtrace_core::insights::{InsightConfig, InsightEngine, InsightType, Severity, Insight};
...
mod tests {
    ...
    fn create_test_state() -> (AppState, TempDir) {
        ...
        let db = Flowtrace::open(&db_path).unwrap();
        ...
        let state = AppState {
            ...
            eval_registry: Arc::new(flowtrace_evals::EvaluatorRegistry::new()),
    ...
    async fn test_query_empty_database() {
        // This test would require mocking the Flowtrace database
...
pub async fn run_cip_evaluation(
    ...
    use flowtrace_evals::evaluators::causal_integrity::*;
    ...
    let llm_adapter: std::sync::Arc<dyn flowtrace_evals::llm_client::LLMClient> = 
        std::sync::Arc::new(crate::llm::LLMClientAdapter::new(llm_client.clone(), default_model.clone()));
    
    // Create embedding adapter
    let embedding_adapter: std::sync::Arc<dyn flowtrace_evals::llm_client::EmbeddingClient> = 
    ...
    let saboteur: std::sync::Arc<dyn flowtrace_evals::evaluators::causal_integrity::saboteur::Perturbator> = 
...
pub async fn fork_trace_state(
    ...
    let target_edge = run_blocking({
        ...
        move || db.get(target_span_id).map(|opt| opt.ok_or_else(|| 
            flowtrace_core::FlowtraceError::NotFound(format!("Span {} not found", target_span_id))
        ))
    }).await?
    .map_err(|_e| CommandError::not_found("span"))?;
    
    // Reconstruct the causal path by backtracking
    let mut path: Vec<flowtrace_core::AgentFlowEdge> = vec![target_edge.clone()];
...
/// Returns the absolute path to the flowtrace-claude-bridge/dist/index.js file
...
pub async fn get_bridge_path() -> Result<String, String> {
    ...
    let possible_paths = vec![
        "flowtrace-claude-bridge/dist/index.js",
        "../flowtrace-claude-bridge/dist/index.js",
        "../../flowtrace-claude-bridge/dist/index.js",
    ];
    ...
    Err("Could not auto-detect flowtrace-claude-bridge path. Please verify it is installed.".to_string())
}
