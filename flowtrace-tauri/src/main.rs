...
// Import Flowtrace crates
use flowtrace_core::{AgentFlowEdge, SavedViewRegistry};
...
pub struct AppState {
    ...
    pub eval_registry: Arc<flowtrace_evals::EvaluatorRegistry>,
    pub eval_store: Arc<flowtrace_storage::EvalStore>,
    ...
    pub online_evaluator: Option<Arc<flowtrace_evals::OnlineEvaluator>>,
...
impl AppConfig {
    pub fn load(app_handle: &tauri::AppHandle) -> Result<Self> {
        // First, check if FLOWTRACE_CONFIG_PATH env var is set (for TOML config)
        if let Ok(toml_path) = std::env::var("FLOWTRACE_CONFIG_PATH") {
            if std::path::Path::new(&toml_path).exists() {
                ...
                tracing::warn!("FLOWTRACE_CONFIG_PATH set but file not found: {}", toml_path);
            }
        }
...
fn initialize_app_state(app_handle: &tauri::AppHandle) -> Result<AppState> {
    ...
    // Open Flowtrace database with high-performance WAL mode
    tracing::info!("Opening Flowtrace database at: {:?}", db_path);
    tracing::info!("Using high-performance WAL mode (Group Commit)");
    let db = Arc::new(Flowtrace::open_high_performance(&db_path)?);
    ...
    let eval_config = flowtrace_evals::EvalConfig {
        max_concurrent: 10,
        timeout_secs: 30,
    ...
    let eval_registry = Arc::new(flowtrace_evals::EvaluatorRegistry::with_config(eval_config));

    // Register built-in local evaluators (no LLM required)
    {
        use flowtrace_evals::evaluators::{LatencyBenchmark, CostAnalyzer, TrajectoryEfficiencyEvaluator};
    ...
    let eval_store = Arc::new(
        flowtrace_storage::EvalStore::open(&eval_store_path)
...
fn main() {
    ...
    let log_dir = std::env::var("HOME")
        ...
        .map(|home| PathBuf::from(home).join(".flowtrace"))
        .unwrap_or_else(|_| PathBuf::from(".flowtrace"));

    std::fs::create_dir_all(&log_dir).ok();
    let log_file = log_dir.join("flowtrace-desktop.log");

    let file_appender = tracing_appender::rolling::never(&log_dir, "flowtrace-desktop.log");

    tracing_subscriber::registry()
        .with(
            tracing_subscriber::EnvFilter::try_from_default_env().unwrap_or_else(|_| {
                "flowtrace=info,flowtrace_query=info,flowtrace_core=info".into()
            }),
        )
    ...
    tracing::info!("Starting Flowtrace Desktop Application");
    ...
    builder
        .setup(|app| {
            ...
            if let Some(window) = app.get_webview_window("main") {
                ...
                // #[cfg(target_os = "macos")]
                // {
                //     match window.to_panel::<FlowtracePanel>() {
                //         Ok(panel) => {
            ...
            tracing::info!("Flowtrace Desktop initialized successfully");
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            commands::get_flowtrace_settings,
            commands::save_flowtrace_settings,
