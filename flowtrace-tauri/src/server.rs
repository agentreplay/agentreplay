...
// flowtrace-tauri/src/server.rs
//! HTTP Server Integration for Tauri Desktop App
//!
//! This module embeds the flowtrace-server HTTP server into the Tauri application,
...
/// Simplified span structure for ingestion (matches flowtrace-observability)
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct FlowtraceSpan {
    pub span_id: String,
    pub trace_id: String,
...
pub async fn start_mcp_server(host: String, port: u16, tauri_state: AppState) -> Result<()> {
    ...
    let server_agent_registry = Arc::new(flowtrace_server::agent_registry::AgentRegistry::new(agent_registry_path));
    let server_cost_tracker = Arc::new(flowtrace_server::cost_tracker::CostTracker::new());
    // Vector index types differ between Tauri and Server; set to None for now
    let vector_index: Option<std::sync::Arc<flowtrace_index::HnswIndex>> = None;

    // Construct Server AppState
    let server_app_state = flowtrace_server::api::AppState {
    ...
    // Create MCP Router
    let mcp_router = flowtrace_server::mcp::mcp_router(server_app_state, causal_index);
...
/// Detailed health check endpoint (compatible with flowtrace-server API)
...
/// Convert FlowtraceSpan to AgentFlowEdge (simplified version from flowtrace-server)
fn convert_span_to_edge(span: &FlowtraceSpan) -> Result<AgentFlowEdge, String> {
    use flowtrace_core::Environment;
    ...
    // Validate timestamps
    let config = flowtrace_core::TimestampConfig::production();
    flowtrace_core::validate_timestamp(span.start_time, &config)
        .map_err(|e| format!("Invalid start_time: {}", e))?;

    if let Some(end_time) = span.end_time {
        flowtrace_core::validate_timestamp(end_time, &config)
...
fn parse_span_name_to_type(name: &str) -> flowtrace_core::SpanType {
...
async fn create_project(
    ...
    let env_vars = EnvVariables {
        flowtrace_url: format!("http://{}:{}", host, port),
...
async fn reset_all_data_handler(
    ...
    // Note: Traces are stored in the Flowtrace database
...
async fn export_backup_handler(
    ...
    // Generate filename with flowtrace_backup_ prefix
    let filename = format!("flowtrace_backup_{}.zip", backup_id.trim_start_matches("backup_"));
...
async fn analyze_trace_handler(
    ...
    span_details.push(serde_json::json!({
        ...
        "type": format!("{:?}", flowtrace_core::SpanType::from_u64(trace.span_type as u64)),
    ...
    for obs in &observations {
        ...
        span_details.push(serde_json::json!({
            ...
            "name": attrs.get("name").and_then(|v| v.as_str())
                ...
                .unwrap_or("Span"),
            "type": format!("{:?}", flowtrace_core::SpanType::from_u64(obs.span_type as u64)),
...
async fn add_custom_pricing_handler(
    ...
    use flowtrace_core::model_pricing::CustomPricingOverride;
            ...
            if let Ok(payload_str) = String::from_utf8(payload_bytes.clone()) {
                if payload_str.to_lowercase().contains(&query_lower) {
                    ...
                    matched.push(SearchResult {
                        ...
                        status: if edge.is_deleted() || matches!(span, flowtrace_core::SpanType::Error) {
                            "error".to_string()
                        } else {
                            "success".to_string()
                        },
...
async fn run_geval_handler(
    ...
    let eval_metric = flowtrace_core::eval::EvalMetric::new(
        trace_id,
        "geval_score",
        overall_score,
        "g-eval",
    ...
    if let Some(label) = dataset_candidate_label {
        ...
        let tag_metric = flowtrace_core::eval::EvalMetric::new(
            trace_id,
            "dataset_candidate",
...
fn extract_trace_io_for_eval(state: &ServerState, trace_id: u128) -> (String, String, String) {
    ...
    let mut all_edges_to_check: Vec<&flowtrace_core::AgentFlowEdge> = trace_edges.clone();
...
fn extract_trace_metadata(db: &Arc<flowtrace_query::Flowtrace>, edge: &AgentFlowEdge) -> (Option<String>, f64) {
...
async fn eval_pipeline_evaluate_handler(
    ...
    eval_run.status = flowtrace_core::RunStatus::Completed;
...
async fn eval_pipeline_recommendations_handler(
    ...
    if !edges.is_empty() {
        ...
        recommendations.push(serde_json::json!({
            ...
            "effort": "low",
            "actions": [
                "Instrument your LLM application with Flowtrace SDK",
