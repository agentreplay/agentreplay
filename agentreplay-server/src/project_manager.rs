...
//! Project Manager - manages multiple Agentreplay instances per project
//!
//! Each project gets its own isolated storage directory:
//! - agentreplay/project_0/wal/
//! - agentreplay/project_0/data/
//! - agentreplay/project_0/causal.index
//! - agentreplay/project_0/vector.index
//!
//! Benefits:
...
use agentreplay_core::{AgentFlowEdge, Result};
...
/// Manages multiple Agentreplay database instances, one per project
///
...
pub struct ProjectManager {
    /// Base directory for all projects (e.g., "./agentreplay")
    base_dir: PathBuf,
    /// LRU cache of open Agentreplay instances: project_id -> Agentreplay
    /// Max 50 projects open at once to prevent FD exhaustion
    projects: Cache<u16, Arc<Agentreplay>>,
}
...
impl ProjectManager {
    ...
    ///
    /// # Arguments
    /// * `base_dir` - Base directory for all project data (e.g., "./agentreplay")
    ///
    /// # Directory Structure
    /// ```text
    /// base_dir/
    ...
    pub fn new<P: AsRef<Path>>(base_dir: P) -> Result<Self> {
        ...
        let projects = Cache::builder()
            ...
            .eviction_listener(|key, _value: Arc<Agentreplay>, cause| {
                // Agentreplay::close() will be called when Arc drops
    ...
    /// Get or open a Agentreplay instance for a project
    ///
    /// This method:
    /// 1. Checks if the project is already open (cached)
    /// 2. If not, opens the Agentreplay database for that project
    /// 3. Caches the instance for future use (LRU eviction policy)
    /// 4. Returns an Arc to the Agentreplay instance
    ///
    /// Thread-safe: Multiple threads can call this concurrently.
    /// The LRU cache automatically evicts least-recently-used projects
    /// when the capacity (50) is reached.
    pub fn get_or_open_project(&self, project_id: u16) -> Result<Arc<Agentreplay>> {
        ...
        self.projects
            .try_get_with(project_id, || {
                // Open the Agentreplay instance for this project
                let project_dir = self.project_dir(project_id);
                info!(
                    "Opening Agentreplay for project {} at {:?}",
                    project_id, project_dir
                );

                // Always use high-performance mode for projects
                Agentreplay::open_high_performance(&project_dir).map(Arc::new)
            })
            .map_err(|e| match Arc::try_unwrap(e) {
                Ok(err) => err,
                Err(arc_err) => {
                    // If we can't unwrap, create a new error with the same message
                    use agentreplay_core::AgentreplayError;
                    AgentreplayError::Internal(format!("Failed to open project: {}", arc_err))
                }
            })
    }
    ...
    pub fn close_all(&self) -> Result<()> {
        ...
        // Agentreplay::close() will be called when Arcs drop
        Ok(())
    }
