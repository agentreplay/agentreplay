...
impl OtlpTraceService {
    ...
    /// Convert OTLP spans to Agentreplay JSON format
    fn convert_to_agentreplay(
        &self,
        ...
        for resource_spans in &request.resource_spans {
            ...
            for scope_spans in &resource_spans.scope_spans {
                for span in &scope_spans.spans {
                    ...
                    // Convert span to Agentreplay format
                    let agentreplay_span = serde_json::json!({
...
impl TraceService for OtlpTraceService {
    async fn export(
        ...
        // Convert to Agentreplay format
        let spans = match self.convert_to_agentreplay(&otlp_request) {
            Ok(spans) => spans,
            Err(e) => {
        ...
        // Get Agentreplay instance for this project
        let agentreplay = match self.project_manager.get_or_open_project(project_id as u16) {
            Ok(ft) => ft,
            Err(e) => {
                error!(
                    "OTLP: Failed to get Agentreplay for tenant={}, project={}: {}",
        ...
        if !entries.is_empty() {
            for (edge, payload) in entries {
                ...
                if let Err(e) = agentreplay.put_payload(edge.edge_id, &payload) {
