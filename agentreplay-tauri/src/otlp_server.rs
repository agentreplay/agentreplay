...
// agentreplay-tauri/src/otlp_server.rs
...
use bytes::Bytes;
use agentreplay_core::{AgentFlowEdge, Environment, ModelPricingRegistry, SpanType};
...
async fn handle_otlp_http(
    ...
    for resource_span in &request.resource_spans {
        ...
        if let Some(pid) = &header_project_id {
            if !resource_attrs.contains_key("project_id") && !resource_attrs.contains_key("agentreplay.project_id") {
                resource_attrs.insert("agentreplay.project_id".to_string(), pid.clone());
            }
        }
...
fn convert_otel_span_to_edge(
    ...
    // Priority: tenant_id > agentreplay.tenant_id > service.namespace > default(1)
    let tenant_id = resource_attrs
        .get("tenant_id")
        .and_then(|s| s.parse::<u64>().ok())
        .or_else(|| {
            resource_attrs
                .get("agentreplay.tenant_id")
                .and_then(|s| s.parse::<u64>().ok())
        })
        .or_else(|| {
            resource_attrs
    ...
    // Priority: project_id > agentreplay.project_id > hash of service.name > default(0)
    let project_id = resource_attrs
        .get("project_id")
        .and_then(|s| s.parse::<u16>().ok())
        .or_else(|| {
            resource_attrs
                .get("agentreplay.project_id")
                .and_then(|s| s.parse::<u16>().ok())
        })
        .or_else(|| {
