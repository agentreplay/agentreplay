  ...
  useEffect(() => {
    ...
    const checkCircuitBreaker = (): boolean => {
      ...
      if (cb.state === 'open') {
        ...
        if (now - cb.lastFailureTime > 30_000) {
          cb.state = 'half-open';
          console.log('[Agentreplay] Circuit breaker transitioning to half-open, attempting connection');
          return true;
        }
        console.log('[Agentreplay] Circuit breaker is open, blocking connection attempt');
        return false;
      }
    ...
    const recordSuccess = () => {
      ...
      if (pendingTracesRef.current.length > 0) {
        console.log(`[Agentreplay] Flushing ${pendingTracesRef.current.length} pending traces`);
    ...
    const recordFailure = (errorType: 'network' | 'auth' | 'unknown') => {
      ...
      if (cb.failureCount >= 5) {
        cb.state = 'open';
        console.error('[Agentreplay] Circuit breaker opened after 5 failures');
        setConnectionState('failed');
      }
    };
    ...
    const connect = () => {
      ...
      if (reconnectAttemptsRef.current >= maxRetries) {
        console.error('[Agentreplay] Max reconnection attempts reached');
        setConnectionState('failed');
      ...
      try {
        ...
        ws.onopen = () => {
          ...
          console.log('[Agentreplay] WebSocket connected');
          setConnected(true);
          setError(null);
        ...
        ws.onmessage = (event) => {
          ...
          try {
            ...
            console.error('[Agentreplay] Failed to parse trace event', err);
            recordFailure('unknown');
          }
        };

        ws.onerror = (evt) => {
          console.error('[Agentreplay] WebSocket error', evt);
          setConnected(false);
        ...
        ws.onclose = (evt) => {
          ...
          setConnected(false);
          setConnectionState('disconnected');
          console.log('[Agentreplay] WebSocket closed', { code: evt.code, reason: evt.reason, wasClean: evt.wasClean });
          ...
          if (!evt.wasClean) {
            ...
            console.log(`[Agentreplay] Reconnecting in ${Math.round(currentBackoffRef.current)}ms (attempt ${reconnectAttemptsRef.current}/${maxRetries})`);
            reconnectTimeoutRef.current = setTimeout(connect, currentBackoffRef.current);
          }
        };
      } catch (err) {
        console.error('[Agentreplay] Failed to establish WebSocket', err);
