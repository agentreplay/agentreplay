...
// flowtrace-observability/src/batcher.rs
//! Async batching exporter for Flowtrace traces
//!
//! This module implements asynchronous batching to minimize latency overhead
//! in hot paths. Spans are collected in memory and sent to Flowtrace's REST API
//! in batches either:
...
/// A simplified span structure for Flowtrace with W3C Trace Context support
#[derive(Debug, Clone, Serialize, Deserialize)]
...
pub struct BatcherConfig {
    ...
    /// Flowtrace endpoint (e.g., "http://localhost:9600")
...
pub struct AsyncBatchExporter {
    sender: mpsc::Sender<FlowtraceSpan>,
...
impl AsyncBatchExporter {
    ...
    pub async fn record_span(&self, span: FlowtraceSpan) {
...
async fn batch_worker(
    mut receiver: mpsc::Receiver<FlowtraceSpan>,
    config: BatcherConfig,
...
/// Flush a batch of spans to Flowtrace's REST API
async fn flush_batch(
    buffer: &mut Vec<FlowtraceSpan>,
    ...
    debug!("Flushing batch of {} spans to Flowtrace", batch_size);

    // Send to Flowtrace's REST API: POST /api/v1/traces
    let endpoint = format!("{}/api/v1/traces", config.flowtrace_endpoint);

    let mut request = client.post(&endpoint).json(&buffer);

    // Add API key if configured
    if let Some(api_key) = &config.api_key {
        request = request.header("X-Flowtrace-API-Key", api_key);
    }

    match request.send().await {
        Ok(response) => {
            if response.status().is_success() {
                debug!("Successfully exported {} spans to Flowtrace", batch_size);
            } else {
                warn!("Failed to export spans: HTTP {}", response.status());
            }
        }
        Err(e) => {
            warn!("Error sending spans to Flowtrace: {}", e);
        }
    }
