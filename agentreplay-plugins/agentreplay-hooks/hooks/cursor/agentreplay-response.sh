#!/bin/bash
# Agent Replay: Agent Response Hook for Cursor
...
if [ "$AGENTREPLAY_HOOK_DEBUG" = "true" ]; then
    echo "$(date -Iseconds) [RESPONSE] INPUT: $INPUT" >> "${AGENTREPLAY_HOOK_LOG:-/tmp/agentreplay-hook.log}"
fi

# Transform payload
TRANSFORMED=$(echo "$INPUT" | jq '. + {session_id: .conversation_id, cwd: .workspace_roots[0], action_type: "agent_response"}' 2>/dev/null || echo "$INPUT")
