...
impl ABTestingEngine {
    ...
    pub fn get_experiment(&self, id: u128) -> Result<ABExperiment> {
        self.experiments
            ...
            .get(&id)
            .cloned()
            .ok_or_else(|| AgentreplayError::NotFound("Experiment not found".into()).into())
    }
    ...
    pub fn assign_variant(&self, experiment_id: u128, session_id: &str) -> Result<String> {
        let experiment = self
            ...
            .get(&experiment_id)
            .cloned()
            .ok_or_else(|| AgentreplayError::NotFound("Experiment not found".into()))?;
    ...
    pub async fn record_result(
        ...
        let should_analyze = {
            ...
            let experiment = experiments
                ...
                .ok_or_else(|| AgentreplayError::NotFound("Experiment".into()))?;

            let variant = experiment
                .variants
                .iter_mut()
                .find(|v| v.id == variant_id)
                .ok_or_else(|| AgentreplayError::NotFound("Variant".into()))?;
...
impl StatisticalAnalyzer {
    pub async fn analyze(&self, experiment: &ABExperiment) -> Result<StatisticalAnalysis> {
        let control = experiment
            ...
            .iter()
            .find(|v| v.is_control)
            .ok_or_else(|| AgentreplayError::InvalidArgument("No control variant".into()))?;
