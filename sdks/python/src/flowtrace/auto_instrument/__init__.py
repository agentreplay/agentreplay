...
def auto_instrument(
    ...
    logger.info(f"🚀 Setting up Flowtrace OpenTelemetry for service: {service_name}")
    
    try:
        # Use the otel_bridge to set up tracer provider
        from flowtrace.otel_bridge import setup_tracer_provider
        ...
        if capture_content:
            os.environ["FLOWTRACE_CAPTURE_CONTENT"] = "true"
        else:
            os.environ["FLOWTRACE_CAPTURE_CONTENT"] = "false"
...
def _auto_instrument_all():
    ...
    import os
    capture_content = os.getenv("FLOWTRACE_CAPTURE_CONTENT", "true").lower() in {"1", "true", "yes"}
    if capture_content:
    ...
    # Try LangChain - use custom Flowtrace callback handler for hierarchy
    try:
        from flowtrace.langchain_tracer import get_flowtrace_callback
        
        callback = get_flowtrace_callback()
        if callback:
...
def setup_instrumentation(
    ...
    service_name = service_name or os.getenv("FLOWTRACE_SERVICE_NAME", "default-service")
    otlp_endpoint = otlp_endpoint or os.getenv("FLOWTRACE_OTLP_ENDPOINT", "localhost:4317")
