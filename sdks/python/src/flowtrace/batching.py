...
from flowtrace.models import AgentFlowEdge


class BatchingFlowtraceClient:
    ...
    **CRITICAL FIX**: Now includes max_buffer_size to prevent OOM.
    ...
    Args:
        client: Underlying FlowtraceClient
    ...
    Example:
        >>> client = FlowtraceClient(url="http://localhost:8080", tenant_id=1)
        >>> batching_client = BatchingFlowtraceClient(
        ...     client,
        ...     batch_size=100,
    ...
    def __enter__(self) -> "BatchingFlowtraceClient":
    ...
    def insert(self, edge: AgentFlowEdge) -> AgentFlowEdge:
        ...
        **CRITICAL FIX**: Now enforces max_buffer_size to prevent OOM.
        ...
        Returns:
            The same edge (for consistency with FlowtraceClient API)
        """
        with self._lock:
