...
"""Environment-based auto-configuration for Flowtrace.
...
Supported environment variables:
    OTEL_EXPORTER_OTLP_ENDPOINT: Flowtrace server URL
    ...
    FLOWTRACE_URL: Override for Flowtrace server URL
    FLOWTRACE_TENANT_ID: Tenant identifier
    FLOWTRACE_PROJECT_ID: Project identifier
    FLOWTRACE_AGENT_ID: Default agent identifier
    FLOWTRACE_AUTO_INSTRUMENT: Enable auto-instrumentation (true/false)
    FLOWTRACE_FRAMEWORKS: Comma-separated list of frameworks to instrument
...
Example:
    ...
    export FLOWTRACE_AUTO_INSTRUMENT="true"
    
    # Python code - zero configuration needed!
    from flowtrace import init_from_env
    init_from_env()
...
class EnvConfig:
    ...
    def __init__(self):
        ...
        self.service_name = os.getenv("OTEL_SERVICE_NAME", "flowtrace-agent")
        self.tenant_id = int(os.getenv("FLOWTRACE_TENANT_ID", "1"))
        self.project_id = self._get_project_id()
        self.agent_id = int(os.getenv("FLOWTRACE_AGENT_ID", "1"))
        
        # Sampling configuration
        self.sampler, self.sampling_rate = self._get_sampling_config()
        
        # Auto-instrumentation configuration
        self.auto_instrument = os.getenv("FLOWTRACE_AUTO_INSTRUMENT", "false").lower() in ("true", "1", "yes")
        self.frameworks = self._get_frameworks()
        
        # Validation
        self.validate_on_init = os.getenv("FLOWTRACE_VALIDATE", "true").lower() in ("true", "1", "yes")
        
        # OTLP compatibility
        self.enable_otlp_export = os.getenv("FLOWTRACE_ENABLE_OTLP", "true").lower() in ("true", "1", "yes")
        
        logger.info(f"EnvConfig loaded: service={self.service_name}, url={self.url}, tenant={self.tenant_id}")
    
    def _get_url(self) -> str:
        """Get Flowtrace server URL from environment."""
        # Priority: FLOWTRACE_URL > OTEL_EXPORTER_OTLP_ENDPOINT > default
    ...
    def _get_project_id(self) -> int:
        ...
        # Try FLOWTRACE_PROJECT_ID first
        project_id_str = os.getenv("FLOWTRACE_PROJECT_ID")
        if project_id_str:
            try:
                return int(project_id_str)
            except ValueError:
                logger.warning(f"Invalid FLOWTRACE_PROJECT_ID: {project_id_str}, using 0")
    ...
    def _get_frameworks(self) -> List[str]:
        ...
        """
        frameworks_str = os.getenv("FLOWTRACE_FRAMEWORKS", "")
    ...
    def validate_connection(self) -> bool:
        """Validate connection to Flowtrace server.
            ...
            for endpoint in ["/health", "/api/v1/health", "/"]:
                try:
                    ...
                    if response.status_code < 500:
                        logger.info(f"✓ Flowtrace server reachable at {self.url}")
                        return True
                except:
                    continue
            
            logger.warning(f"✗ Cannot reach Flowtrace server at {self.url}")
            return False
    ...
    def create_client(self) -> FlowtraceClient:
        """Create a FlowtraceClient from this configuration.
        
        Returns:
            Configured FlowtraceClient
        """
...
def init_from_env(
    ...
    This is the recommended way to set up Flowtrace for production deployments.
    ...
    Example:
        ...
        >>> os.environ["FLOWTRACE_TENANT_ID"] = "1"
        >>> 
    ...
    if validate and config.validate_on_init:
        if not config.validate_connection():
            raise ConnectionError(
                f"Cannot reach Flowtrace server at {config.url}\n"
                f"Troubleshooting:\n"
                f"  1. Check server is running: curl {config.url}/health\n"
                f"  2. Verify firewall/network settings\n"
                f"  3. Set FLOWTRACE_URL or OTEL_EXPORTER_OTLP_ENDPOINT\n"
    ...
    if should_auto_instrument:
        try:
            from flowtrace.auto_instrument import auto_instrument as do_auto_instrument
            
            frameworks = config.frameworks if config.frameworks else None
            
            do_auto_instrument(
                service_name=config.service_name,
                flowtrace_url=config.url,
    ...
    logger.info("✓ Flowtrace initialized from environment")
