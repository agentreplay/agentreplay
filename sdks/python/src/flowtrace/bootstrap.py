...
This module is called by the .pth file on Python startup when FLOWTRACE_ENABLED=true.
...
Environment Variables:
    ...
    FLOWTRACE_SERVICE_NAME: Service name for traces (default: 'flowtrace-app')
    FLOWTRACE_OTLP_ENDPOINT: OTLP gRPC endpoint (default: 'localhost:4317')
    FLOWTRACE_PROJECT_ID: Project ID for traces
    FLOWTRACE_TENANT_ID: Tenant ID for traces (default: 1)
    FLOWTRACE_DEBUG: Enable debug logging (default: false)
    FLOWTRACE_CAPTURE_CONTENT: Capture LLM request/response content (default: true)
    OTEL_EXPORTER_OTLP_ENDPOINT: Standard OTEL endpoint override

Example:
    # Option 1: Automatic via .pth file
    $ export FLOWTRACE_ENABLED=true
    $ export FLOWTRACE_PROJECT_ID=27986
...
def init_otel_instrumentation(
    ...
    Args:
        service_name: Service name (default: from env or 'flowtrace-app')
    ...
    if _initialized:
        logger.debug("Flowtrace already initialized, skipping")
        return False
    
    # Read from environment with fallbacks
    service_name = service_name or os.getenv("FLOWTRACE_SERVICE_NAME", "flowtrace-app")
    otlp_endpoint = otlp_endpoint or os.getenv(
        "FLOWTRACE_OTLP_ENDPOINT",
        os.getenv("OTEL_EXPORTER_OTLP_ENDPOINT", "localhost:4317")
    )
    
    # Project/tenant IDs
    if project_id is None:
        project_id_str = os.getenv("FLOWTRACE_PROJECT_ID", "0")
        try:
    ...
    if tenant_id is None:
        tenant_id_str = os.getenv("FLOWTRACE_TENANT_ID", "1")
        try:
            tenant_id = int(tenant_id_str)
        except ValueError:
            logger.warning(f"Invalid FLOWTRACE_TENANT_ID: {tenant_id_str}, using 1")
            tenant_id = 1
    
    # Flags
    if capture_content is None:
        capture_content = os.getenv("FLOWTRACE_CAPTURE_CONTENT", "true").lower() in {
            "1", "true", "yes"
        }
    
    if debug is None:
        debug = os.getenv("FLOWTRACE_DEBUG", "false").lower() in {"1", "true", "yes"}
    ...
    try:
        ...
        logger.info(f"🚀 Initializing Flowtrace for service: {service_name}")
        ...
        _initialized = True
        logger.info("✅ Flowtrace initialization complete")
        return True
        
    except Exception as e:
        logger.error(f"❌ Failed to initialize Flowtrace: {e}", exc_info=debug)
...
def _auto_init():
    ...
    Only initializes if FLOWTRACE_ENABLED=true to avoid overhead.
    ...
    if not os.getenv("FLOWTRACE_ENABLED", "").lower() in {"1", "true", "yes"}:
    ...
    try:
        ...
        import sys
        print(f"Flowtrace auto-init failed: {e}", file=sys.stderr)
        pass


def is_initialized() -> bool:
    """Check if Flowtrace has been initialized.
