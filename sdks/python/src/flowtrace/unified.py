...
backends (Flowtrace, LangSmith, Langfuse, Opik) based on environment variables.
...
Example:
    ...
    export OBSERVABILITY_BACKEND="flowtrace"  # or "langsmith", "langfuse", "opik"
    
    # Python code - works with any backend!
    from flowtrace.unified import UnifiedObservability
...
class FlowtraceBackend(ObservabilityBackend):
    """Flowtrace observability backend."""
    
    def __init__(self, **config):
        """Initialize Flowtrace backend.
        
        Args:
            **config: Configuration passed to FlowtraceClient
        """
        from flowtrace import FlowtraceClient
        
        url = config.get("url", os.getenv("FLOWTRACE_URL", "http://localhost:8080"))
        tenant_id = config.get("tenant_id", int(os.getenv("FLOWTRACE_TENANT_ID", "1")))
        project_id = config.get("project_id", int(os.getenv("FLOWTRACE_PROJECT_ID", "0")))
        
        self.client = FlowtraceClient(
            url=url,
            tenant_id=tenant_id,
            project_id=project_id,
        )
        
        logger.info("FlowtraceBackend initialized")
    
    @contextmanager
    def trace(self, name: str, **kwargs):
        """Create Flowtrace span."""
    ...
    def set_attribute(self, span: Any, key: str, value: Any) -> None:
        """Set attribute on Flowtrace span."""
        # Flowtrace uses specific methods for common attributes
...
class UnifiedObservability:
    ...
    - OBSERVABILITY_BACKEND: Backend name (flowtrace, langsmith, langfuse, opik)
    ...
    def __init__(self, backend_name: Optional[str] = None, **config):
        ...
        Args:
            backend_name: Override backend selection (flowtrace, langsmith, langfuse, opik)
    ...
    def _auto_detect_backend(self) -> str:
        ...
        if backend in ("flowtrace", "langsmith", "langfuse", "opik"):
            return backend
    ...
    def _create_backend(self, backend_name: str, **config) -> ObservabilityBackend:
        ...
        """
        if backend_name == "flowtrace":
            return FlowtraceBackend(**config)
            ...
            raise ValueError(
                ...
                f"Supported: flowtrace, langsmith, langfuse, opik"
            )
...
class MultiBackend:
    ...
    Example:
        >>> from flowtrace.unified import MultiBackend
        >>> 
        >>> # Send to both Flowtrace and LangSmith
        >>> obs = MultiBackend(["flowtrace", "langsmith"])
        >>> 
