...
Example:
    >>> from flowtrace.propagation import inject_trace_context, extract_trace_context
    >>> 
...
def inject_trace_context(
    ...
    flowtrace_tenant_id: Optional[int] = None,
    flowtrace_project_id: Optional[int] = None,
) -> None:
    ...
    Args:
        ...
        flowtrace_tenant_id: Optional tenant ID for tracestate
        flowtrace_project_id: Optional project ID for tracestate
    ...
    if flowtrace_tenant_id is not None:
        tracestate_parts.append(f"flowtrace=t{flowtrace_tenant_id}")
    
    if flowtrace_project_id is not None:
        if tracestate_parts:
            # Append to existing flowtrace state
            tracestate_parts[0] += f"p{flowtrace_project_id}"
        else:
            tracestate_parts.append(f"flowtrace=p{flowtrace_project_id}")
...
class TraceContextPropagator:
    ...
    def inject(self, headers: Dict[str, str]) -> None:
        ...
        inject_trace_context(
            ...
            flowtrace_tenant_id=self.tenant_id,
            flowtrace_project_id=self.project_id,
        )
