...
class FlowtraceClient:
    ...
    def __enter__(self) -> "FlowtraceClient":
    ...
    def track_llm_call(
        ...
        Example:
            >>> from flowtrace.genai import GenAIAttributes
            >>> 
            >>> # For OpenAI
            ...
            >>> client.track_llm_call(genai_attrs)
        """
        # Convert GenAI attributes to Flowtrace span format
        import time
    ...
    def insert(self, edge: AgentFlowEdge) -> AgentFlowEdge:
        ...
        # Convert edge to FlowtraceSpan format expected by server
        span = {
...
class AsyncFlowtraceClient:
    """Async version of FlowtraceClient for high-performance applications.
    
    Provides the same API as FlowtraceClient but with async/await support.
    """
    ...
    def __init__(
        ...
        """Initialize async Flowtrace client."""
        self.url = url.rstrip("/")
        self.tenant_id = tenant_id
        self.project_id = project_id
        self.agent_id = agent_id
    ...
    async def __aenter__(self) -> "AsyncFlowtraceClient":
