...
"""Bridge between Flowtrace spans and OpenTelemetry spans.

This module provides utilities to convert Flowtrace's custom span format
to OpenTelemetry format and vice versa. It also handles the setup of
the OpenTelemetry tracer provider with Flowtrace-specific configuration.

The bridge ensures that:
1. Flowtrace spans are compatible with standard OTLP exporters
2. Agent/session/workflow context is preserved in span attributes
3. Flowtrace's span types map to appropriate OTEL span kinds
"""
...
def setup_tracer_provider(
    ...
    """Set up OpenTelemetry tracer provider with Flowtrace configuration.
    
    Args:
        service_name: Service name for resource attributes
        otlp_endpoint: OTLP gRPC endpoint (e.g., 'localhost:4317')
        tenant_id: Flowtrace tenant ID
        project_id: Flowtrace project ID
    ...
    Example:
        ...
        ... )
    """
    # Create resource with Flowtrace-specific attributes
    ...
    resource = Resource.create({
        ...
        "flowtrace.sdk.name": "flowtrace-python",
        "flowtrace.sdk.version": "0.1.0",
    })
    ...
    headers = {
        "x-flowtrace-tenant-id": str(tenant_id),
        "x-flowtrace-project-id": str(project_id),
    }
...
def flowtrace_span_type_to_otel_kind(span_type: str) -> SpanKind:
    """Convert Flowtrace span type to OTEL span kind.
    
    Args:
        span_type: Flowtrace span type (e.g., 'Planning', 'Reasoning', 'ToolCall')
    
    Returns:
        Appropriate SpanKind
    """
    # Map Flowtrace types to OTEL span kinds
    mapping = {
...
def inject_agent_context_to_span(span: trace.Span, context: Dict[str, Any]):
    """Inject Flowtrace agent context into an OTEL span.
    ...
    if "agent_id" in context:
        span.set_attribute("flowtrace.agent_id", context["agent_id"])
    
    if "session_id" in context:
        span.set_attribute("flowtrace.session_id", context["session_id"])
    
    if "workflow_id" in context:
        span.set_attribute("flowtrace.workflow_id", context["workflow_id"])
    
    if "user_id" in context:
        span.set_attribute("flowtrace.user_id", context["user_id"])
...
def _inject_agent_context(span: trace.Span):
    ...
    try:
        ...
        if agent_id:
            span.set_attribute("flowtrace.agent_id", agent_id)
        
        session_id = get_current_session_id()
        if session_id:
            span.set_attribute("flowtrace.session_id", session_id)
        
        workflow_id = get_current_workflow_id()
        if workflow_id:
            span.set_attribute("flowtrace.workflow_id", workflow_id)
        
        user_id = get_current_user_id()
        if user_id:
            span.set_attribute("flowtrace.user_id", user_id)
...
def get_tracer(name: str = "flowtrace") -> trace.Tracer:
    """Get an OpenTelemetry tracer for Flowtrace.
    
    Args:
        name: Tracer name (default: 'flowtrace')
