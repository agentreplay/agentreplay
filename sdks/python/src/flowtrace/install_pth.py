...
"""Install the flowtrace-init.pth file to site-packages for auto-initialization.

This script copies the .pth file that enables zero-code auto-instrumentation.
Run after pip install: python -m flowtrace.install_pth

Or use the CLI command: flowtrace-install
"""
...
PTH_CONTENT = """import flowtrace.bootstrap; flowtrace.bootstrap._auto_init()
"""

PTH_FILENAME = "flowtrace-init.pth"
...
def get_site_packages():
    ...
    # Also check where flowtrace itself is installed
    try:
        import flowtrace
        flowtrace_dir = os.path.dirname(flowtrace.__file__)
        site_packages = os.path.dirname(flowtrace_dir)
...
def install():
    ...
    print("\n🎉 Flowtrace auto-instrumentation is now enabled!")
    print("   Set FLOWTRACE_ENABLED=true to activate tracing.")
    print("   Set FLOWTRACE_PROJECT_ID=<id> to specify your project.")
    return True
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Install Flowtrace auto-instrumentation .pth file"
    )
    parser.add_argument(
        "--uninstall", "-u",
        action="store_true",
