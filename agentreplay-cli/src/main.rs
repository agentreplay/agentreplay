...
//! Agentreplay CLI
//!
//! Command-line interface for Agentreplay database operations.

use anyhow::{Context, Result};
use clap::{Parser, Subcommand};
use agentreplay_core::{AgentFlowEdge, SpanType};
use agentreplay_plugins::{PluginConfig, PluginManager, UninstallMode};
...
#[derive(Parser)]
#[command(name = "agentreplay")]
#[command(about = "Agentreplay - AgentFlow Format Database", long_about = None)]
struct Cli {
    /// Database directory
    #[arg(short, long, default_value = "./agentreplay-data")]
    db_path: PathBuf,
...
async fn main() -> Result<()> {
    ...
    // Open database
    let db = Agentreplay::open(&cli.db_path).context("Failed to open database")?;
    ...
    match cli.command {
        ...
        Commands::Stats => {
            ...
            println!("Agentreplay Statistics");
            println!("=====================");
            println!();
            println!("Memory:");
            println!(
...
async fn handle_plugin_command(
    ...
    match command {
        ...
        PluginCommands::Init {
            ...
            let manifest_path = plugin_dir.join("agentreplay-plugin.toml");
            ...
            if json_output {
                ...
                println!("  4. agentreplay plugin install .");
            }
        }
        ...
        PluginCommands::Build {
            ...
            let manifest_path = path.join("agentreplay-plugin.toml");
            if !manifest_path.exists() {
                eprintln!("✗ No agentreplay-plugin.toml found in {}", path.display());
                std::process::exit(1);
            }
        ...
        PluginCommands::Validate { path } => {
            ...
            let manifest_path = path.join("agentreplay-plugin.toml");
            if !manifest_path.exists() {
                eprintln!("✗ No agentreplay-plugin.toml found");
                std::process::exit(1);
            }
            ...
            match toml::from_str::<agentreplay_plugins::PluginManifest>(&manifest_content) {
                Ok(manifest) => {
        ...
        PluginCommands::Package { path, output } => {
            ...
            let manifest: agentreplay_plugins::PluginManifest = toml::from_str(&manifest_content)?;
            ...
            // Add manifest
            tar.append_path_with_name(&manifest_path, "agentreplay-plugin.toml")?;
        ...
        PluginCommands::Run {
            ...
            let wasm_path = if path.extension().map(|e| e == "wasm").unwrap_or(false) {
                ...
                let manifest_path = path.join("agentreplay-plugin.toml");
                if !manifest_path.exists() {
                    eprintln!("✗ No agentreplay-plugin.toml found and path is not a WASM file");
                    std::process::exit(1);
                }
                let manifest_content = std::fs::read_to_string(&manifest_path)?;
                let manifest: agentreplay_plugins::PluginManifest =
            ...
            if !wasm_path.exists() {
                ...
                eprintln!(
                    "  Try running 'agentreplay plugin build {}' first",
                    path.display()
                );
                std::process::exit(1);
            }
...
fn get_entry_path(manifest: &agentreplay_plugins::PluginManifest) -> Option<String> {
...
fn generate_plugin_manifest(
    ...
    let desc = description.unwrap_or("A Agentreplay plugin");
    let auth = author.unwrap_or("Agentreplay User");
...
path = "dist/plugin.wasm""#
    ...
    format!(
        r#"# Agentreplay Plugin Manifest
# Generated by agentreplay plugin init
...
"#
            ...
            // Generate lib.rs
            let lib_rs = format!(
                r#"//! {name} - Agentreplay Evaluator Plugin
...
A Agentreplay evaluator plugin written in Rust.
...
"#
        ...
        "python-evaluator" => {
            ...
            let evaluator_py = format!(
                r#"\"\"\"
{name} - Agentreplay Evaluator Plugin
\"\"\"
...
name = "{id}"
version = "0.1.0"
description = "{name} Agentreplay plugin"
...
A Agentreplay evaluator plugin written in Python.
...
A Agentreplay evaluator plugin written in TypeScript.
    ...
    match command {
        ...
        BackupCommands::Export { backup_id, output } => {
            ...
            let output_path = output.unwrap_or_else(|| {
                PathBuf::from(format!("agentreplay_backup_{}.zip", 
